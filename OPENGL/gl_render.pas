...
var
  position_indicators: array[1..5] of integer = (-1, -1, -1, -1, -1);

function gld_BindPositionIndicatorTex(const position: integer): boolean;
var
  gltex: PGLTexture;
begin
  Result := False;

  if not IsIntegerInRange(position, 1, 5) then
    Exit;

  if position_indicators[position] < 0 then
    position_indicators[position] := W_CheckNumForName('RACEPOS' + itoa(position - 1));
  if position_indicators[position] < 0 then
    Exit;

  gltex := gld_RegisterPatch(position_indicators[position], Ord(CR_DEFAULT));
  if gltex = nil then
    Exit;

  gld_BindPatch(gltex, Ord(CR_DEFAULT));

  Result := True;
end;

const
  POS_INDICATOR_SIZE = 32;
  POS_INDICATOR_OFFS = 20;
