...
type
  ...
  public
    ...
    property IsEvent: Boolean read FIsEvent write FIsEvent;
    property IsTask: Boolean read FIsTask write FIsTask;
    constructor Create;
    destructor Destroy; override;
  end;
...
{$ENDIF}
  ...
  TPSPascalCompilerErrorType = (
    ...
    ecInvalidEventParameters,
    ecTaskVarParameters,
    ecEventsCanNotBeUsedAsVariables
    {$IFDEF PS_USESSUPPORT}
    , ecNotAllowed,
  ...
  public
    ...
    function MakeErrorEx(const Module: TbtString; E: TPSPascalCompilerErrorType; const
    ...
    function AddDelphiFunctionWithRTLObjectResult(const Decl: TbtString; const fresult: TbtString): TPSRegProc;
    ...
    function Compile(const s: TbtString): Boolean; virtual;
    function GetOutput(var s: TbtString): Boolean; virtual;
    function GetDebugOutput(var s: TbtString): Boolean; virtual;
    procedure Clear;
    ...
    {$WARNINGS ON}
    property Classes: TPSList read FClasses;
    property keywords: PRTabArray read fkeywords;
    property numkeywords: integer read fnumkeywords;
  end;
  ...
  public
    ...
    property RTLClassName: TbtString read FClassName;
  end;
  ...
  private
    ...
    FNameHash: Longint;
    FDecl: TPSParametersDecl;
    FDeclaration: TbtString;
    procedure SetName(const s: TbtString);
  public
    constructor Create(Owner: TPSCompileTimeClass; const aDeclaration: TbtString);
    destructor Destroy; override;
    property Decl: TPSParametersDecl read FDecl;
    property Name: TbtString read FName;
    property OrgName: TbtString read FOrgName write SetName;
    ...
    property RTLDeclaration: TbtString read FDeclaration;
  end;
  ...
  public
    constructor Create(Owner: TPSPascalCompiler; InheritedFrom: TPSInterface;
...
{$ENDIF}
  ...
  RPS_InvalidEventParameters = 'Event parameters do not match template';
  RPS_TaskVarParameters = 'Task declaration can not have var/out parameters';
  RPS_EventsCanNotBeUsedAsVariables = 'Can not declare type event';
...
begin
  ...
  case p.FType.BaseType of
    btType:
      BlockWriteData(BlockInfo, p^.ttype.FinalTypeno, 4);
    {$IFNDEF PS_NOWIDESTRING}
    btWideString:
      begin
    ...
    btWideChar:
      BlockWriteData(BlockInfo, p^.twidechar, 2);
    {$ENDIF}
    btSingle:
      BlockWriteData(BlockInfo, p^.tsingle, SizeOf(TbtSingle));
    btDouble:
      BlockWriteData(BlockInfo, p^.tdouble, SizeOf(TbtDouble));
    btExtended:
      BlockWriteData(BlockInfo, p^.textended, SizeOf(TbtExtended));
    btCurrency:
      BlockWriteData(BlockInfo, p^.tcurrency, SizeOf(TbtCurrency));
    btChar:
      BlockWriteData(BlockInfo, p^.tchar, 1);
    btSet:
      begin
    ...
    btS8,
    btU8:
      BlockWriteData(BlockInfo, p^.tu8, 1);
    btS16,
    btU16:
      BlockWriteData(BlockInfo, p^.tu16, 2);
    btS32,
    btU32:
      BlockWriteData(BlockInfo, p^.tu32, 4);
    {$IFNDEF PS_NOINT64}
    bts64:
      BlockWriteData(BlockInfo, p^.ts64, 8);
    {$ENDIF}
    btProcPtr:
      BlockWriteData(BlockInfo, p^.tu32, 4);
    {$IFDEF DEBUG}
...
begin
  ...
  begin
    Parser := TPSPascalParser.Create(Owner.keywords, Owner.numkeywords);
    Parser.SetText(Decl);
  end
  else
    Parser := CustomParser;
  if Parser.CurrTokenId = CSTII_Function then
    FuncType:= 0
          ...
          (Parser.CurrTokenId = CSTII_Event) or
          (Parser.CurrTokenId = CSTII_Task) then
  begin
    FuncType := 1;
    if Parser.CurrTokenId = CSTII_Event then
      isEvent := True
    else if Parser.CurrTokenId = CSTII_Task then
      isTask := True;
  end
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if Parser <> CustomParser then
              ...
              Owner.MakeError('', ecTaskVarParameters, '');
            Result := False;
            Exit;
          end;
          Parser.Next;
        ...
        begin
          ...
          begin
            ...
            case VCType.BaseType of
              btU8:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFU8', 'array of byte');
              btS8:
              ...
              btS32:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFS32', 'array of Longint');
              btSingle:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFSINGLE', 'array of Single');
              btDouble:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFDOUBLE', 'array of Double');
              btExtended:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFEXTENDED', 'array of Extended');
              btString:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFSTRING', 'array of String');
              btPChar:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFPCHAR', {$IFDEF PS_PANSICHAR}'array of PAnsiChar'{$ELSE}'array of PChar'{$ENDIF});
              btNotificationVariant,
              btVariant:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFVARIANT', 'array of variant');
              {$IFNDEF PS_NOINT64}
              btS64:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFS64', 'array of Int64');
              {$ENDIF}
              btChar:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFCHAR', 'array of Char');
              {$IFNDEF PS_NOWIDESTRING}
              btWideString:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFWIDESTRING', 'array of WideString');
              btUnicodeString:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFUNICODESTRING', 'array of UnicodeString');
              btWideChar:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFWIDECHAR', 'array of WideChar');
              {$ENDIF}
              btClass:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFTOBJECT', 'array of TObject');
              btRecord:
                VCType := FindAndAddType(Owner, '!OPENARRAYOFRECORD_' + FastUpperCase(Parser.OriginalToken), 'array of ' + FastUpperCase(Parser.OriginalToken));
            else
              begin
                if Parser <> CustomParser then
                  Parser.Free;
        ...
        else if Parser.CurrTokenId = CSTII_Const then
          VCType := nil // any type
        else
        begin
...
begin
  ...
  case BaseType of
    btProcPtr:
      Result := TPSProceduralType.Create;
    btTypeCopy:
      Result := TPSTypeLink.Create;
    btRecord:
      Result := TPSRecordType.Create;
    btArray:
      Result := TPSArrayType.Create;
    btStaticArray:
      Result := TPSStaticArrayType.Create;
    btEnum:
      Result := TPSEnumType.Create;
    btClass:
      Result := TPSClassType.Create;
    btExtClass:
      Result := TPSUndefinedClassType.Create;
    btNotificationVariant,
    btVariant:
      Result := TPSVariantType.Create;
    {$IFNDEF PS_NOINTERFACES}
    btInterface:
      Result := TPSInterfaceType.Create;
    {$ENDIF}
  else
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if isTask then
            if modifier in [pmInOut, pmOut] then
              raise EPSCompilerException.Create(RPS_TaskVarParameters);
...
begin
  case src.FType.BaseType of
    ...
    btS32:
      Dest^.tu32 := src^.tu32;
    btSingle:
      Dest^.tsingle := src^.tsingle;
    btDouble:
      Dest^.tdouble := src^.tdouble;
    btExtended:
      Dest^.textended := src^.textended;
    btCurrency:
      Dest^.tcurrency := Src^.tcurrency;
    btChar:
      Dest^.tchar := src^.tchar;
    {$IFNDEF PS_NOINT64}
      bts64:
        Dest^.ts64 := src^.ts64;
    {$ENDIF}
    btset,
    btstring:
      TbtString(Dest^.tstring) := TbtString(src^.tstring);
    {$IFNDEF PS_NOWIDESTRING}
    btUnicodeString:
      TbtUnicodeString(Dest^.tunistring) := TbtUnicodeString(src^.tunistring);
    btWideString:
      TbtWideString(Dest^.twidestring) := TbtWideString(src^.twidestring);
    btwidechar:
      Dest^.tchar := src^.tchar;
    {$ENDIF}
  end;
...
begin
  case Src.FType.BaseType of
    ...
    {$ENDIF}
    btChar:
      Result := ord(Src^.tchar);
    {$IFNDEF PS_NOWIDESTRING}
    btWideChar:
      Result := ord(TbtWideChar(src^.twidechar));
    {$ENDIF}
    btEnum:
      Result := src^.tu32;
  else
...
begin
  case Src.FType.BaseType of
    ...
    btDouble:
      Result := Src^.tdouble;
    btCurrency:
      Result := Src^.tcurrency;
    btExtended:
      Result := Src^.textended;
    else
      begin
        s := False;
        Result := 0;
...
begin
  case Src.FType.BaseType of
    btChar:
      Result := Src^.tchar;
    btString:
      Result := TbtString(Src^.tstring);
    {$IFNDEF PS_NOWIDESTRING}
    btWideChar:
      Result := TbtString(Src^.twidechar);
    btWideString:
      Result := TbtString(TbtWideString(Src^.twidestring));
    btUnicodeString:
      Result := TbtString(TbtUnicodeString(Src^.tunistring));
    {$ENDIF}
    else
      begin
        s := False;
...
begin
  case Src.FType.BaseType of
    btChar:
      Result := TbtWideString(Src^.tchar);
    btString:
      Result := TbtWideString(TbtString(src^.tstring));
    btWideChar:
      Result := src^.twidechar;
    btWideString:
      Result := TbtWideString(src^.twidestring);
    btUnicodeString:
      Result := TbtUnicodeString(src^.tunistring);
    else
      begin
        s := False;
        Result := '';
...
begin
  case Src.FType.BaseType of
    btChar:
      Result := TbtUnicodeString(Src^.tchar);
    btString:
      Result := TbtUnicodeString(TbtString(src^.tstring));
    btWideChar:
      Result := src^.twidechar;
    btWideString:
      Result := TbtWideString(src^.twidestring);
    ...
    case Cmd of
      otAdd:
        begin { + }
          case var1.FType.BaseType of
            btU8:
              var1^.tu8 := var1^.tu8 + GetUint(Var2, Result);
            btS8:
              var1^.ts8 := var1^.ts8 + GetInt(Var2, Result);
            btU16:
              var1^.tu16 := var1^.tu16 + GetUint(Var2, Result);
            btS16:
              var1^.ts16 := var1^.ts16 + Getint(Var2, Result);
            btEnum,
            btU32:
              var1^.tu32 := var1^.tu32 + GetUint(Var2, Result);
            btS32:
              var1^.ts32 := var1^.ts32 + Getint(Var2, Result);
            {$IFNDEF PS_NOINT64}
            btS64:
              var1^.ts64 := var1^.ts64 + GetInt64(Var2, Result);
            {$ENDIF}
            btSingle:
              var1^.tsingle := var1^.tsingle + GetReal(Var2, Result);
            btDouble:
              var1^.tdouble := var1^.tdouble + GetReal(Var2, Result);
            btExtended:
              var1^.textended := var1^.textended + GetReal(Var2, Result);
            btCurrency:
              var1^.tcurrency := var1^.tcurrency + GetReal(Var2, Result);
            btSet:
              begin
            ...
            btString:
              TbtString(var1^.tstring) := TbtString(var1^.tstring) + GetString(Var2, Result);
            {$IFNDEF PS_NOWIDESTRING}
            btWideString:
              TbtWideString(var1^.twidestring) := TbtWideString(var1^.twidestring) + GetWideString(Var2, Result);
            btUnicodeString:
      ...
      otIntDiv:
        begin { / }
          case Var1.FType.BaseType of
            btU8:
              var1^.tu8 := var1^.tu8 div GetUint(Var2, Result);
            btS8:
              var1^.ts8 := var1^.ts8 div Getint(Var2, Result);
            btU16:
              var1^.tu16 := var1^.tu16 div GetUint(Var2, Result);
            btS16:
              var1^.ts16 := var1^.ts16 div Getint(Var2, Result);
            btU32:
              var1^.tu32 := var1^.tu32 div GetUint(Var2, Result);
            btS32:
              var1^.ts32 := var1^.ts32 div Getint(Var2, Result);
            {$IFNDEF PS_NOINT64}
            btS64:
              var1^.ts64 := var1^.ts64 div GetInt64(Var2, Result);
            {$ENDIF}
            else
              Result := False;
          end;
      ...
      otMod:
        begin { MOD }
          case Var1.FType.BaseType of
            btU8:
              var1^.tu8 := var1^.tu8 mod GetUint(Var2, Result);
            btS8:
              var1^.ts8 := var1^.ts8 mod Getint(Var2, Result);
            btU16:
              var1^.tu16 := var1^.tu16 mod GetUint(Var2, Result);
            btS16:
              var1^.ts16 := var1^.ts16 mod Getint(Var2, Result);
            btU32:
              var1^.tu32 := var1^.tu32 mod GetUint(Var2, Result);
            btS32:
              var1^.ts32 := var1^.ts32 mod Getint(Var2, Result);
            {$IFNDEF PS_NOINT64}
            btS64:
              var1^.ts64 := var1^.ts64 mod GetInt64(Var2, Result);
            {$ENDIF}
            else
              Result := False;
          end;
      ...
      otshl:
        begin { SHL }
          case Var1.FType.BaseType of
            btU8:
              var1^.tu8 := var1^.tu8 shl GetUint(Var2, Result);
            btS8:
              var1^.ts8 := var1^.ts8 shl Getint(Var2, Result);
            btU16:
              var1^.tu16 := var1^.tu16 shl GetUint(Var2, Result);
            btS16:
              var1^.ts16 := var1^.ts16 shl Getint(Var2, Result);
            btU32:
              var1^.tu32 := var1^.tu32 shl GetUint(Var2, Result);
            btS32:
              var1^.ts32 := var1^.ts32 shl Getint(Var2, Result);
            {$IFNDEF PS_NOINT64}
            btS64:
              var1^.ts64 := var1^.ts64 shl GetInt64(Var2, Result);
            {$ENDIF}
            else
              Result := False;
          end;
      ...
      otShr:
        begin { SHR }
          case Var1.FType.BaseType of
            btU8:
              var1^.tu8 := var1^.tu8 shr GetUint(Var2, Result);
            btS8:
              var1^.ts8 := var1^.ts8 shr Getint(Var2, Result);
            btU16:
              var1^.tu16 := var1^.tu16 shr GetUint(Var2, Result);
            btS16:
              var1^.ts16 := var1^.ts16 shr Getint(Var2, Result);
            btU32:
              var1^.tu32 := var1^.tu32 shr GetUint(Var2, Result);
            btS32:
              var1^.ts32 := var1^.ts32 shr Getint(Var2, Result);
            {$IFNDEF PS_NOINT64}
            btS64:
              var1^.ts64 := var1^.ts64 shr GetInt64(Var2, Result);
            {$ENDIF}
            else
              Result := False;
          end;
      ...
      otAnd:
        begin { AND }
          case Var1.FType.BaseType of
            btU8:
              var1^.tu8 := var1^.tu8 and GetUint(Var2, Result);
            btS8:
              var1^.ts8 := var1^.ts8 and Getint(Var2, Result);
            btU16:
              var1^.tu16 := var1^.tu16 and GetUint(Var2, Result);
            btS16:
              var1^.ts16 := var1^.ts16 and Getint(Var2, Result);
            btU32:
              var1^.tu32 := var1^.tu32 and GetUint(Var2, Result);
            btS32:
              var1^.ts32 := var1^.ts32 and Getint(Var2, Result);
            btEnum:
              var1^.ts32 := var1^.ts32 and Getint(Var2, Result);
            {$IFNDEF PS_NOINT64}
            btS64:
              var1^.ts64 := var1^.ts64 and GetInt64(Var2, Result);
            {$ENDIF}
            else
              Result := False;
          end;
      ...
      otor:
        begin { OR }
          case Var1.FType.BaseType of
            btU8:
              var1^.tu8 := var1^.tu8 or GetUint(Var2, Result);
            btS8:
              var1^.ts8 := var1^.ts8 or Getint(Var2, Result);
            btU16:
              var1^.tu16 := var1^.tu16 or GetUint(Var2, Result);
            btS16:
              var1^.ts16 := var1^.ts16 or Getint(Var2, Result);
            btU32:
              var1^.tu32 := var1^.tu32 or GetUint(Var2, Result);
            btS32:
              var1^.ts32 := var1^.ts32 or Getint(Var2, Result);
            {$IFNDEF PS_NOINT64}
            btS64:
              var1^.ts64 := var1^.ts64 or GetInt64(Var2, Result);
            {$ENDIF}
            btEnum:
              var1^.ts32 := var1^.ts32 or Getint(Var2, Result);
            else
      ...
      otxor:
        begin { XOR }
          case Var1.FType.BaseType of
            btU8:
              var1^.tu8 := var1^.tu8 xor GetUint(Var2, Result);
            btS8:
              var1^.ts8 := var1^.ts8 xor Getint(Var2, Result);
            btU16:
              var1^.tu16 := var1^.tu16 xor GetUint(Var2, Result);
            btS16:
              var1^.ts16 := var1^.ts16 xor Getint(Var2, Result);
            btU32:
              var1^.tu32 := var1^.tu32 xor GetUint(Var2, Result);
            btS32:
              var1^.ts32 := var1^.ts32 xor Getint(Var2, Result);
            {$IFNDEF PS_NOINT64}
            btS64:
              var1^.ts64 := var1^.ts64 xor GetInt64(Var2, Result);
            {$ENDIF}
            btEnum:
              var1^.ts32 := var1^.ts32 xor Getint(Var2, Result);
            else
      ...
      otGreaterEqual:
        begin { >= }
          case Var1.FType.BaseType of
            ...
            btS8:
              b := var1^.ts8 >= Getint(Var2, Result);
            btU16:
              b := var1^.tu16 >= GetUint(Var2, Result);
            btS16:
              b := var1^.ts16 >= Getint(Var2, Result);
            btU32:
              b := var1^.tu32 >= GetUint(Var2, Result);
            btS32:
              b := var1^.ts32 >= Getint(Var2, Result);
            {$IFNDEF PS_NOINT64}
            btS64:
              b := var1^.ts64 >= GetInt64(Var2, Result);
            {$ENDIF}
            btSingle:
              b := var1^.tsingle >= GetReal(Var2, Result);
            btDouble:
              b := var1^.tdouble >= GetReal(Var2, Result);
            btExtended:
              b := var1^.textended >= GetReal(Var2, Result);
            btCurrency:
              b := var1^.tcurrency >= GetReal(Var2, Result);
            btSet:
              begin
                if (var1.FType = var2.FType) then
                begin
        ...
        begin { <> }
          case Var1.FType.BaseType of
            ...
            btEnum:
              b := var1^.ts32 <> Getint(Var2, Result);
            btString:
              b := TbtString(var1^.tstring) <> GetString(var2, Result);
            btChar:
              b := var1^.tchar <> GetString(var2, Result);
            {$IFNDEF PS_NOWIDESTRING}
            btWideString:
              b := TbtWideString(var1^.twidestring) <> GetWideString(var2, Result);
            btUnicodeString:
              b := TbtUnicodeString(var1^.tunistring) <> GetUnicodeString(var2, Result);
            btWideChar:
              b := var1^.twidechar <> GetUnicodeString(var2, Result);
            {$ENDIF}
            btSet:
              begin
                if (var1.FType = var2.FType) then
        ...
        begin { = }
          case Var1.FType.BaseType of
            ...
            btWideString:
              b := TbtWideString(var1^.twidestring) = GetWideString(var2, Result);
            btUnicodeString:
              b := TbtUnicodeString(var1^.twidestring) = GetUnicodeString(var2, Result);
            btWideChar:
              b := var1^.twidechar = GetUnicodeString(var2, Result);
            {$ENDIF}
            btSet:
...
begin
  ...
  begin
    if FParser.CurrTokenId = CSTII_Event then
      isEvent := True
    else if FParser.CurrTokenId = CSTII_Task then
      isTask := True;
    IsFunction := False;
  end;
  ...
  begin
    if FParser = Self.FParser then
      MakeError('', ecEventsCanNotBeUsedAsVariables, '');
    Result := nil;
    Exit;
  end;
...
var
  ...
  {$ENDIF}
begin
  if (FParser.CurrTokenId = CSTII_Function) or
     (FParser.CurrTokenId = CSTII_Procedure) or
     (FParser.CurrTokenId = CSTII_Event) or
     (FParser.CurrTokenId = CSTII_Task) then
  begin
     Result := ReadTypeAddProcedure(Name, FParser);
     Exit;
  end
  else if FParser.CurrTokenId = CSTII_Set then
  begin
    FParser.Next;
  ...
  else if FParser.CurrTokenId = CSTI_OpenRound then
  begin
    FParser.Next;
    L := 0;
  ...
  else if FParser.CurrTokenId = CSTII_Array then
  begin
    FParser.Next;
    ...
    begin
      ...
      case tempf.FType.BaseType of
        btU8:
          FArrayStart := tempf.tu8;
        btS8:
          FArrayStart := tempf.ts8;
        btU16:
          FArrayStart := tempf.tu16;
        btS16:
          FArrayStart := tempf.ts16;
        btU32:
          FArrayStart := tempf.tu32;
        btS32:
          FArrayStart := tempf.ts32;
        {$IFNDEF PS_NOINT64}
        bts64:
          FArrayStart := tempf.ts64;
        {$ENDIF}
      else
        begin
      ...
      case tempf.FType.BaseType of
        btU8:
          FArrayLength := tempf.tu8;
        btS8:
          FArrayLength := tempf.ts8;
        btU16:
          FArrayLength := tempf.tu16;
        btS16:
          FArrayLength := tempf.ts16;
        btU32:
          FArrayLength := tempf.tu32;
        btS32:
          FArrayLength := tempf.ts32;
        {$IFNDEF PS_NOINT64}
        bts64:
          FArrayLength := tempf.ts64;
        {$ENDIF}
      else
        DisposeVariant(tempf);
        MakeError('', ecTypeMismatch, '');
  ...
  else if FParser.CurrTokenId = CSTII_Interface then
  begin
    FParser.Next;
    if FParser.CurrTokenId <> CSTI_OpenRound then
    begin
    ...
    FParser.Next;
    repeat
      if not Intf.RegisterMethodEx('', cdStdCall, FParser) then
      begin
  ...
  else if FParser.CurrTokenId = CSTI_Identifier then
  begin
    s := FParser.GetToken;
    h := MakeHash(s);
    Typeno := nil;
...
begin
  ...
  if (FParser.CurrTokenId = CSTII_Procedure) or
     (FParser.CurrTokenId = CSTII_Event) or
     (FParser.CurrTokenId = CSTII_Task) then
  begin
    FunctionType := ftProc;
  ...
  try
    ...
    else if (FParser.CurrTokenId = CSTII_Forward) or AlwaysForward then
    begin
      if Func <> nil then
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (PType = nil) or
           ((PType.BaseType = btArray) and (TPSArrayType(PType).ArrayTypeNo = nil) and (GetTypeNo(BlockInfo, Params[c].Val).BaseType = btArray)) or
        ...
        else if (PType.BaseType = btArray) and (GetTypeNo(BlockInfo, Params[c].Val).BaseType = btArray) then
        begin
        ...
        else if IsVarInCompatible(PType, GetTypeNo(BlockInfo, Params[c].Val)) then
        begin
          MakeError('', ecTypeMismatch, '');
          Result := False;
          Exit;
...
begin
  ...
  repeat
    ...
    if (FParser.CurrTokenId = CSTII_Procedure) or
       (FParser.CurrTokenId = CSTII_Function) or
       (FParser.CurrTokenId = CSTII_Event) or
       (FParser.CurrTokenId = CSTII_Task) then
    begin
...
function TPSPascalCompiler.ProcessSub(BlockInfo: TPSBlockInfo): Boolean;
  ...
  begin
    ...
    begin
      if ((p1 <> nil) and
          ({$IFNDEF PS_NOINTERFACES}(p1.ClassType = TPSInterfaceType) or {$ENDIF}(p1.BaseType = btProcPtr)) and
          (v2.ClassType = TPSValueNil)) or
        ((p2 <> nil) and
         ({$IFNDEF PS_NOINTERFACES}(p2.ClassType = TPSInterfaceType) or {$ENDIF}(p2.BaseType = btProcPtr)) and
         (v1.ClassType = TPSValueNil)) then
      begin
        Result := True;
        Exit;
      end
      else if ((p1 <> nil) and
               ({$IFNDEF PS_NOINTERFACES}(p1.ClassType = TPSInterfaceType) or {$ENDIF}(p1.ClassType = TPSClassType)) and
               (v2.ClassType = TPSValueNil)) or
              ((p2 <> nil) and
               ({$IFNDEF PS_NOINTERFACES}(p2.ClassType = TPSInterfaceType) or {$ENDIF}(p2.ClassType = TPSClassType)) and
               (v1.ClassType = TPSValueNil)) then
      begin
        Result := True;
        Exit;
               ...
               ({$IFNDEF PS_NOINTERFACES}(p1.ClassType = TPSInterfaceType) or {$ENDIF}(p1.ClassType = TPSUndefinedClassType)) and
               (v2.ClassType = TPSValueNil)) or
              ((p2 <> nil) and
               ({$IFNDEF PS_NOINTERFACES}(p2.ClassType = TPSInterfaceType) or {$ENDIF}(p2.ClassType = TPSUndefinedClassType)) and
               (v1.ClassType = TPSValueNil)) then
      begin
        Result := True;
        Exit;
      end
      else if (v1.ClassType = TPSValueProcPtr) and (p2 <> nil) and (p2.BaseType = btProcPtr) then
      begin
        Result := CheckCompatProc(p2, TPSValueProcPtr(v1).ProcPtr);
        Exit;
      end
      else if (v2.ClassType = TPSValueProcPtr) and (p1 <> nil) and (p1.BaseType = btProcPtr) then
      begin
    ...
    else if (p1 <> nil) and (p1.BaseType = btSet) and (v2 is TPSValueArray) then
    begin
      Result := True;
    end
    else
  ...
  begin
    ...
    if (Temp.ResultType <> nil) and (Temp.ResultType = FAnyString) then
    begin // workaround to make the Result type match
  ...
  begin
    ...
    else if (pf.BaseType = btString) or (pf.BaseType = btPChar) then
    begin
    ...
    else if (pf.BaseType = btClass) {$IFNDEF PS_NOINTERFACES}or (pf.BaseType = btInterface){$ENDIF} then
    begin
      {$IFNDEF PS_NOINTERFACES}
    ...
    else if pf.BaseType = btExtClass then
    begin
  ...
  begin
    ...
    begin
      ...
      case BVal.Operator of
        ...
        otNotEqual:
          BlockWriteByte(BlockInfo, 4);
        otIn:
          BlockWriteByte(BlockInfo, 6);
        otIs:
          BlockWriteByte(BlockInfo, 7);
      end;
    ...
    begin
      if (BVal.aType <> nil) and (BVal.aType <> GetTypeNo(BlockInfo, Output)) then
      begin
      ...
      begin
        ...
        else if BVal.Operator = otOr then
        begin
  ...
  begin
    ...
    else if x.ClassType = TPSValueProcPtr then
    begin
    ...
    else if x.ClassType = TPSValueNil then
    begin
      if FArrType = nil then
      begin
        MakeError('', ecTypeMismatch, '');
    ...
    else if x.ClassType = TPSValueArray then
    begin
      if FArrType = nil then
      begin
        MakeError('', ecTypeMismatch, '');
    ...
    else if (x.ClassType = TPSUnValueOp) then
    begin
    ...
    else if (x.ClassType = TPSBinValueOp) then
    begin
      tmpp := AllocStackReg(GetTypeNo(BlockInfo, x));
    ...
    else if x is TPSValueProc then
    begin
    ...
    else if (x is TPSValueVar) and (TPSValueVar(x).RecCount <> 0) then
    begin
      ...
      begin
        ...
        begin // if rr.classtype = TPSSubValue then begin
  ...
  begin
    ...
    else if x.ClassType = TPSValueData then
    begin
      if AllowData then
      begin
        BlockWriteByte(BlockInfo, 1);
  ...
  var
    ...
    tmp: TPSValue;
  begin
    if not (Val is TPSValueGlobalVar) then
      Exit;
    aType := GetTypeNo(BlockInfo, Val);
    if (AType = nil) or (AType.BaseType <> btNotificationVariant) then
      Exit;
...
(*      if not (x is TPSValueVar) then
      ...
      begin
        ...
        {$IFNDEF PS_NOIDISPATCH}or ((u.BaseType = btNotificationVariant)){$ENDIF} or (u.BaseType = btExtClass) then
          Exit;
        if FParser.CurrTokenId = CSTI_OpenBlock then
        begin
          ...
          begin
            ...
            begin
              ...
              {$IFDEF DEBUG}
              if not Param.ExpectedType.Used then
                asm int 3; end;
              {$ENDIF}
            end
            else
          ...
          else if ((u.BaseType = btArray) or (u.BaseType = btStaticArray)) and (x is TPSValueVar) then
          begin
            FParser.Next;
            tmp := calc(CSTI_CloseBlock);
            if tmp = nil then
         ...
         {$IFDEF PS_HAVEVARIANT}and not (u.BaseType = btVariant){$ENDIF} then
        begin
          ...
          {$IFDEF PS_HAVEVARIANT}
          else if (u.BaseType = btVariant) then
            Break
          else
          {$ELSE}
          ;
    ...
    begin
      if p = nil then
        Exit;
      if (GetTypeNo(BlockInfo, p) = nil) or (GetTypeNo(BlockInfo, p).BaseType <> btClass) then
        Exit;
    ...
    begin
      ...
      begin
        ...
        ImplicitPeriod := False;
        FType := GetTypeNo(BlockInfo, p);
        if (FType = nil) or (FType.BaseType <> btExtClass) then
          Exit;
      end; {while}
    end;
    ...
    begin
      ...
      begin
        ...
        else if TPSClassType(FType).cl.Property_Find(s, Idx) then
        begin
    ...
    begin
      ...
      if FType = nil then
        Exit;
      if (FType.BaseType <> btInterface) and
         (Ftype.BaseType <> BtVariant) and
         (FType.BaseType = btNotificationVariant) then
        Exit;
      ...
      begin
        ...
        begin
          ...
          if (FType = nil) or (FType.BaseType <> btInterface) then
            Exit;
        end
        else
        begin
          ...
          FType := GetTypeNo(BlockInfo, p);
          if (FType = nil) or
             ((FType.BaseType <> btInterface) and (Ftype.BaseType <> btVariant) and (Ftype.BaseType <> btNotificationVariant)) then
            Exit;
        end;
      end; {while}
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          vt := ivtVariable;
          if Temp = TWith then
            CheckFurther(TWith, True);
          if Temp = TWith then
            CheckClass(TWith, vt, vno, True);
          if Temp = TWith then
            CheckExtClass(TWith, vt, vno, True);
          if (Temp <> TWith) or (Cardinal(lOldRecCount) <> TPSValueVar(TWith).GetRecCount) then
          begin
            repeat
              Temp := TWith;
              if TWith <> nil then
                CheckFurther(TWith, False);
              if TWith <> nil then
                CheckClass(TWith, vt, vno, False);
              if TWith <> nil then
                CheckExtClass(TWith, vt, vno, False);
              {$IFNDEF PS_NOIDISPATCH}
              if TWith <> nil then
                CheckIntf(TWith, vt, vno, False);
              {$ENDIF}
              if TWith <> nil then
                CheckProcCall(TWith);
              if TWith <> nil then
                CheckClassArrayProperty(TWith, vt, vno);
              vno := InvalidVal;
    ...
    begin
      ...
      begin
        ...
        repeat
          Temp := Result;
          if Result <> nil then
            CheckFurther(Result, False);
          if Result <> nil then
            CheckClass(Result, vt, vno, False);
          if Result <> nil then
            CheckExtClass(Result, vt, vno, False);
          {$IFNDEF PS_NOIDISPATCH}
          if Result <> nil then
            CheckIntf(Result, vt, vno, False);
          {$ENDIF}
          if Result <> nil then
            CheckProcCall(Result);
          if Result <> nil then
            CheckClassArrayProperty(Result, vt, vno);
          vno := InvalidVal;
    ...
    begin
      if (TPSVar(FVars[l]).NameHash = h) and
         (TPSVar(FVars[l]).Name = s)
         {$IFDEF PS_USESSUPPORT} and (IsInLocalUnitList(TPSVar(FVars[l]).FDeclareUnit)){$ENDIF} then
      begin
        TPSVar(FVars[l]).Use;
        ...
        repeat
          Temp := Result;
          if Result <> nil then
            CheckNotificationVariant(Result);
          if Result <> nil then
            CheckFurther(Result, False);
          if Result <> nil then
            CheckClass(Result, vt, vno, False);
  ...
  function calc(endOn: TPSPasToken): TPSValue;
      ...
      case fParser.CurrTokenId of
        ...
        CSTI_Identifier:
          begin
            ...
            else if FParser.GetToken = 'PRED' then
            begin
              FParser.Next;
              if FParser.CurrTokenId <> CSTI_OpenRound then
              begin
            ...
            else if FParser.GetToken = 'ASSIGNED' then
            begin
              FParser.Next;
              if FParser.CurrTokenId <> CSTI_OpenRound then
              begin
      ...
      case Cmd of
        otAdd: {plus}
          begin
            if ((t1.BaseType = btVariant) or (t1.BaseType = btNotificationVariant)) and (
              ...
              Result := t1
            else if ((t2.BaseType = btVariant) or (t2.BaseType = btNotificationVariant)) and (
        ...
        otLessEqual: { >=, <=, >, <}
          begin
            ...
            else if ((t1.BaseType = btSet) and (t2.BaseType = btSet)) and
                    (t1 = t2) and ((cmd = otGreaterEqual) or (cmd = otLessEqual)) then
              Result := FDefaultBoolType
            else
            if ((t2.BaseType = btVariant) or (t2.BaseType = btNotificationVariant)) and (
              ((t1.BaseType = btVariant) or (t1.BaseType = btNotificationVariant)) or
    ...
    begin
      ...
      begin
        ...
        if (Op = otAs) and (F2 is TPSValueData) and (TPSValueData(F2).Data.FType.BaseType = btType) then
        begin
    ...
    begin
      ...
      else if p is TPSUnValueOp then
      begin
          ...
          case TPSUnValueOp(p).Operator of
            otNot:
              begin
                case TPSValueData(TPSUnValueOp(p).FVal1).Data^.FType.BaseType of
                  ...
                  btU8:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8 := not TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8;
                  btU16:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16 := not TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16;
                  btU32:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu32 := not TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu32;
                  btS8:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts8 := not TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts8;
                  btS16:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts16 := not TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts16;
                  btS32:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts32 := not TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts32;
                  {$IFNDEF PS_NOINT64}
                  bts64:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64 := not TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64;
                  {$ENDIF}
                else
                  begin
                    MakeError('', ecTypeMismatch, '');
            ...
            otMinus:
              begin
                case TPSValueData(TPSUnValueOp(p).FVal1).Data^.FType.BaseType of
                  btU8:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8 := -TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8;
                  btU16:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16 := -TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16;
                  btU32:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu32 := -TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu32;
                  btS8:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts8 := -TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts8;
                  btS16:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts16 := -TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts16;
                  btS32:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts32 := -TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts32;
                  {$IFNDEF PS_NOINT64}
                  bts64:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64 := -TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64;
                  {$ENDIF}
                  btSingle:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.tsingle := -TPSValueData(TPSUnValueOp(p).FVal1).Data^.tsingle;
                  btDouble:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.tdouble := -TPSValueData(TPSUnValueOp(p).FVal1).Data^.tdouble;
                  btExtended:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.textended := -TPSValueData(TPSUnValueOp(p).FVal1).Data^.textended;
                  btCurrency:
                    TPSValueData(TPSUnValueOp(p).FVal1).Data^.tcurrency := -TPSValueData(TPSUnValueOp(p).FVal1).Data^.tcurrency;
                else
                  begin
                    MakeError('', ecTypeMismatch, '');
                    Result := False;
                ...
                case TPSUnValueOp(p).FType.BaseType of
                  btU8:
                    begin
                      case TPSValueData(TPSUnValueOp(p).FVal1).Data.Ftype.basetype of
                        ...
                        {$ENDIF}
                        btU8:
                          TPSValueData(preplace).Data.tu8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8;
                        btS8:
                          TPSValueData(preplace).Data.tu8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS8;
                        btU16:
                          TPSValueData(preplace).Data.tu8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16;
                        btS16:
                          TPSValueData(preplace).Data.tu8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS16;
                        btU32:
                          TPSValueData(preplace).Data.tu8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tU32;
                        btS32:
                          TPSValueData(preplace).Data.tu8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS32;
                        {$IFNDEF PS_NOINT64}
                        btS64:
                          TPSValueData(preplace).Data.tu8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64;
                        {$ENDIF}
                      else
                        begin
                          MakeError('', ecTypeMismatch, '');
                  ...
                  btS8:
                    begin
                      case TPSValueData(TPSUnValueOp(p).FVal1).Data.Ftype.basetype of
                        btChar:
                          TPSValueData(preplace).Data.ts8 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tchar);
                        {$IFNDEF PS_NOWIDESTRING}
                        btwidechar:
                          TPSValueData(preplace).Data.ts8 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.twidechar);
                        {$ENDIF}
                        btU8:
                          TPSValueData(preplace).Data.ts8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8;
                        btS8:
                          TPSValueData(preplace).Data.ts8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS8;
                        btU16:
                          TPSValueData(preplace).Data.ts8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16;
                        btS16:
                          TPSValueData(preplace).Data.ts8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS16;
                        btU32:
                          TPSValueData(preplace).Data.ts8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tU32;
                        btS32:
                          TPSValueData(preplace).Data.ts8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS32;
                        {$IFNDEF PS_NOINT64}
                        btS64:
                          TPSValueData(preplace).Data.ts8 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64;
                        {$ENDIF}
                      else
                        begin
                          MakeError('', ecTypeMismatch, '');
                  ...
                  btU16:
                    begin
                      case TPSValueData(TPSUnValueOp(p).FVal1).Data.Ftype.basetype of
                        btChar:
                          TPSValueData(preplace).Data.tu16 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tchar);
                        {$IFNDEF PS_NOWIDESTRING}
                        btwidechar:
                          TPSValueData(preplace).Data.tu16 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.twidechar);
                        {$ENDIF}
                        btU8:
                          TPSValueData(preplace).Data.tu16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8;
                        btS8:
                          TPSValueData(preplace).Data.tu16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS8;
                        btU16:
                          TPSValueData(preplace).Data.ts16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16;
                        btS16:
                          TPSValueData(preplace).Data.tu16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS16;
                        btU32:
                          TPSValueData(preplace).Data.tu16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tU32;
                        btS32:
                          TPSValueData(preplace).Data.tu16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS32;
                        {$IFNDEF PS_NOINT64}
                        btS64:
                          TPSValueData(preplace).Data.tu16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64;
                        {$ENDIF}
                      else
                        begin
                          MakeError('', ecTypeMismatch, '');
                  ...
                  btS16:
                    begin
                      case TPSValueData(TPSUnValueOp(p).FVal1).Data.Ftype.basetype of
                        btChar:
                          TPSValueData(preplace).Data.ts16 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tchar);
                        {$IFNDEF PS_NOWIDESTRING}
                        btwidechar:
                          TPSValueData(preplace).Data.ts16 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.twidechar);
                        {$ENDIF}
                        btU8:
                          TPSValueData(preplace).Data.ts16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8;
                        btS8:
                          TPSValueData(preplace).Data.ts16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS8;
                        btU16:
                          TPSValueData(preplace).Data.ts16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16;
                        btS16:
                          TPSValueData(preplace).Data.ts16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS16;
                        btU32:
                          TPSValueData(preplace).Data.ts16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tU32;
                        btS32:
                          TPSValueData(preplace).Data.ts16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS32;
                        {$IFNDEF PS_NOINT64}
                        btS64:
                          TPSValueData(preplace).Data.ts16 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64;
                        {$ENDIF}
                      else
                        begin
                          MakeError('', ecTypeMismatch, '');
                  ...
                  btU32:
                    begin
                      case TPSValueData(TPSUnValueOp(p).FVal1).Data.Ftype.basetype of
                        btChar:
                          TPSValueData(preplace).Data.tu32 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tchar);
                        {$IFNDEF PS_NOWIDESTRING}
                        btwidechar:
                          TPSValueData(preplace).Data.tu32 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.twidechar);
                        {$ENDIF}
                        btU8:
                          TPSValueData(preplace).Data.tu32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8;
                        btS8:
                          TPSValueData(preplace).Data.tu32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS8;
                        btU16:
                          TPSValueData(preplace).Data.tu32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16;
                        btS16:
                          TPSValueData(preplace).Data.tu32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS16;
                        btU32:
                          TPSValueData(preplace).Data.tu32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tU32;
                        btS32:
                          TPSValueData(preplace).Data.tu32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS32;
                        {$IFNDEF PS_NOINT64}
                        btS64:
                          TPSValueData(preplace).Data.tu32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64;
                        {$ENDIF}
                      else
                        begin
                          MakeError('', ecTypeMismatch, '');
                  ...
                  btS32:
                    begin
                      case TPSValueData(TPSUnValueOp(p).FVal1).Data.Ftype.basetype of
                        btChar:
                          TPSValueData(preplace).Data.ts32 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tchar);
                        {$IFNDEF PS_NOWIDESTRING}
                        btwidechar:
                          TPSValueData(preplace).Data.ts32 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.twidechar);
                        {$ENDIF}
                        btU8:
                          TPSValueData(preplace).Data.ts32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8;
                        btS8:
                          TPSValueData(preplace).Data.ts32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS8;
                        btU16:
                          TPSValueData(preplace).Data.ts32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16;
                        btS16:
                          TPSValueData(preplace).Data.ts32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS16;
                        btU32:
                          TPSValueData(preplace).Data.ts32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tU32;
                        btS32:
                          TPSValueData(preplace).Data.ts32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS32;
                        {$IFNDEF PS_NOINT64}
                        btS64:
                          TPSValueData(preplace).Data.tu32 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64;
                        {$ENDIF}
                  ...
                  btS64:
                    begin
                      case TPSValueData(TPSUnValueOp(p).FVal1).Data.Ftype.basetype of
                        btChar:
                          TPSValueData(preplace).Data.ts64 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tchar);
                        {$IFNDEF PS_NOWIDESTRING}
                        btwidechar:
                          TPSValueData(preplace).Data.ts64 := ord(TPSValueData(TPSUnValueOp(p).FVal1).Data^.twidechar);
                        {$ENDIF}
                        btU8:
                          TPSValueData(preplace).Data.ts64 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8;
                        btS8:
                          TPSValueData(preplace).Data.ts64 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS8;
                        btU16:
                          TPSValueData(preplace).Data.ts64 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16;
                        btS16:
                          TPSValueData(preplace).Data.ts64 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS16;
                        btU32:
                          TPSValueData(preplace).Data.ts64 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tU32;
                        btS32:
                          TPSValueData(preplace).Data.ts64 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS32;
                        btS64:
                          TPSValueData(preplace).Data.ts64 := TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64;
                      else
                        begin
                          MakeError('', ecTypeMismatch, '');
                          preplace.Free;
                  ...
                  btChar:
                    begin
                      case TPSValueData(TPSUnValueOp(p).FVal1).Data.Ftype.basetype of
                        btChar:
                          TPSValueData(preplace).Data.tchar := TPSValueData(TPSUnValueOp(p).FVal1).Data^.tchar;
                        btU8:
                          TPSValueData(preplace).Data.tchar := TbtChar(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu8);
                        btS8:
                          TPSValueData(preplace).Data.tchar := TbtChar(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS8);
                        btU16:
                          TPSValueData(preplace).Data.tchar := TbtChar(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tu16);
                        btS16:
                          TPSValueData(preplace).Data.tchar := TbtChar(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS16);
                        btU32:
                          TPSValueData(preplace).Data.tchar := TbtChar(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tU32);
                        btS32:
                          TPSValueData(preplace).Data.tchar := TbtChar(TPSValueData(TPSUnValueOp(p).FVal1).Data^.tS32);
                        {$IFNDEF PS_NOINT64}
                        btS64:
                          TPSValueData(preplace).Data.tchar := TbtChar(TPSValueData(TPSUnValueOp(p).FVal1).Data^.ts64);
                        {$ENDIF}
                      else
                        begin
                          MakeError('', ecTypeMismatch, '');
  ...
  begin
    ...
    repeat
      Temp := Val;
      if Val <> nil then
        CheckFurther(Val, False);
      if Val <> nil then
        CheckClass(Val, vt, InvalidVal, False);
      if Val <> nil then
        CheckExtClass(Val, vt, InvalidVal, False);
      {$IFNDEF PS_NOIDISPATCH}
      if Val <> nil then
        CheckIntf(Val, vt, InvalidVal, False);
      {$ENDIF}
      if Val <> nil then
        CheckProcCall(Val);
      if Val <> nil then
        CheckClassArrayProperty(Val, vt, InvalidVal);
  ...
  begin
    ...
    try
      ...
      TPSValueProcNo(Result).ProcNo :=
        aVariantType.GetDynInvokeProcNo(Self, ProcName, TPSValueProcNo(Result).Parameters);
    finally
      Par.Free;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Copy(tmp.ExpectedType.Name, 1, 10) = '!OPENARRAY' then
        begin
...
//          BlockWriteByte(BlockInfo, CM_PO); // pop the temp var
  ...
  begin
    ...
    begin
      ...
      else if vin is TPSValueProc then
      begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else if FParser.CurrTokenId <> CSTII_End then
          begin
            MakeError('', ecEndExpected, '');
            Result := False;
            Exit;
      ...
      else if FParser.CurrTokenId = CSTII_Finally then
      begin
        FParser.Next;
        ...
        begin
          case _ProcessLabel of
            ...
            begin
              ...
              else if FParser.GetToken = 'CONTINUE' then
              begin
                if FBreakOffsets = nil then
                begin
                  MakeError('', ecNotInLoop, '');
              ...
              else if not ProcessIdentifier then
                Exit;
              if (BlockInfo.SubType = tifOneliner) or (BlockInfo.SubType = TOneLiner) then
                Break;
            end;
...
begin
  ...
  begin
    ...
    case p.BaseType of
      btStaticArray,
      btArray:
        TPSArrayType(p).ArrayTypeNo := at2ut(TPSArrayType(p).ArrayTypeNo);
      btRecord:
        begin
      ...
      btSet:
        TPSSetType(p).SetType := at2ut(TPSSetType(p).SetType);
      btProcPtr:
        begin
  ...
  function MakeOutput: Boolean;
    ...
    begin
      ...
      case p.FType.BaseType of
        ...
        btWideChar:
          WriteData(p^.twidechar, 2);
        {$ENDIF}
        btSingle:
          WriteData(p^.tsingle, SizeOf(TbtSingle));
        btDouble:
          WriteData(p^.tsingle, SizeOf(TbtDouble));
        btExtended:
          WriteData(p^.tsingle, SizeOf(TbtExtended));
        btCurrency:
          WriteData(p^.tsingle, SizeOf(TbtCurrency));
        btChar:
          WriteData(p^.tchar, 1);
        btSet:
          begin
    ...
    begin
      ...
      try
        ...
        begin
          ...
          else if (x.BaseType = btEnum) then
          begin
          ...
          begin
            ...
            WriteByte(bt);
          {$IFNDEF PS_NOINTERFACES}
          if x.BaseType = btInterface then
          begin
          ...
          else if x.BaseType = btProcPtr then
          begin
            s := DeclToBits(TPSProceduralType(x).ProcDef);
            WriteLong(Length(s));
            WriteData(s[1], Length(s));
          end
          else if x.BaseType = btSet then
          begin
            WriteLong(TPSSetType(x).BitSize);
          end
          else if (x.BaseType = btArray) or (x.basetype = btStaticArray) then
          begin
...
begin
  ...
  begin
    ...
    OldIsUnit := FIsUnit;
    OldUnit := fUnit;
    FParser := TPSPascalParser.Create(fkeywords, fnumkeywords);
    FParser.SetText(s);
  end;
  ...
  repeat
    ...
    else if (Fparser.CurrTokenId = CSTII_Implementation) and
            ((Position = csinterface) or (position = csInterfaceUses)) then
    begin
      Position := csImplementation;
      FParser.Next;
    end
    else if (Fparser.CurrTokenId = CSTII_Interface) and (Position = csUnit) then
    begin
      Position := csInterface;
      FParser.Next;
    end
    else if (FParser.CurrTokenId = CSTII_Unit) and (Position = csStart) and (FAllowUnit) then
    begin
      Position := csUnit;
      FIsUnit := True;
      FParser.Next;
    ...
    else if (FParser.CurrTokenId = CSTII_Procedure) or
            (FParser.CurrTokenId = CSTII_Function) or
            (FParser.CurrTokenId = CSTII_Event) or
            (FParser.CurrTokenId = CSTII_Task) or
            (FParser.CurrTokenId = CSTI_OpenBlock) then
    begin
      if (Position = csInterface) or (position = csInterfaceUses) then
      begin
...
begin
  ...
  FParser := TPSPascalParser.Create(akeywords, anumkeywords);
...
begin
  ...
  AddType('resourcestring', btUnicodeString);
  AddType('NativeString', btUnicodeString);
  {$ELSE}
  AddType('String', btString);
  AddType('resourcestring', btString);
...
//        MakeError('', ecTypeMismatch, '');
    ...
    else if p is TUnConstOperation then
    begin
        ...
        case OpType of
          ...
          otMinus:
            case p1.FType.BaseType of
              ...
              {$ENDIF}
              btDouble:
                p1.tdouble := -p1.tDouble;
              btSingle:
                p1.tsingle := -p1.tsingle;
              btCurrency:
                p1.tcurrency := -p1.tcurrency;
              btExtended:
                p1.textended := -p1.textended;
            else
              begin
                MakeError('', ecTypeMismatch, '');
                DisposeVariant(p1);
...
begin
  ...
  AddFunction('function StrToInt(s: String): Int64;');
  AddFunction('function StrToIntDef(s: String; def: Int64): Int64;');
  {$ELSE}
  ...
  {$ENDIF}
  with AddFunction('procedure Dec;').Decl do
  begin
    with AddParam do
    begin
      OrgName := 'x';
  ...
  with AddFunction('procedure Inc;').Decl do
  begin
    with AddParam do
    begin
      OrgName := 'x';
  ...
  with AddFunction('procedure Include;').Decl do
  begin
    with AddParam do
    begin
      OrgName := 's';
  ...
  with AddFunction('procedure Exclude;').Decl do
  begin
    with AddParam do
    begin
      OrgName := 's';
...
begin
  if FProcs = nil then
    raise EPSCompilerException.Create(RPS_OnUseEventOnly);
  if FType = nil then
    raise EPSCompilerException.CreateFmt(RPS_InvalidTypeForVar, [Name]);
  s := FastUpperCase(Name);
  if IsDuplicate(s,[dcVars]) then
    raise EPSCompilerException.CreateFmt(RPS_DuplicateIdent, [Name]);
...
begin
  if FAttributeTypes = nil then
    raise Exception.Create(RPS_OnUseEventOnly);
...
begin
  ...
  case Kind of
    ICOMMENTERROR:
      MakeError('', ecCommentError, '');
    ISTRINGERROR:
      MakeError('', ecStringError, '');
    ICHARERROR:
      MakeError('', ecCharError, '');
  else
    MakeError('', ecSyntaxError, '');
  end;
end;
...
begin
  ...
  try
    ...
    if pDecl.Result <> nil then
      raise EPSCompilerException.CreateFmt('Setting RTL result requires procedure declaration [%s]', [Decl]);

    pDecl.Result := FindType(fresult);
...
begin
  cls := FastUpperCase(aClass);
  H := MakeHash(cls);
  for i := 0 to FClasses.Count - 1 do
  begin
    x := FClasses[I];
    if (x.FClassNameHash = H) and (x.FClassName = cls) then
    begin
      Result := x;
      Exit;
    end;
  ...
  begin
    case FValue.FType.BaseType of
      btChar:
        FValue.tchar := c;
      btString:
        TbtString(FValue.tstring) := c;
      {$IFNDEF PS_NOWIDESTRING}
      btWideString:
        TbtWideString(FValue.twidestring) := TbtWideString(c);
      btUnicodeString:
        TbtUnicodeString(FValue.twidestring) := TbtUnicodeString(c);
      {$ENDIF}
    else
  ...
  begin
    case FValue.FType.BaseType of
      ...
      btDouble:
        FValue.tdouble := Val;
      btExtended:
        FValue.textended := Val;
      btCurrency:
        FValue.tcurrency := Val;
    else
      raise EPSCompilerException.Create(RPS_ConstantValueMismatch);
    end;
  end
...
begin
  ...
  begin
    case FValue.FType.BaseType of
      btChar:
        FValue.tchar := (Val + #0)[1];
      btString:
        TbtString(FValue.tstring) := val;
      {$IFNDEF PS_NOWIDESTRING}
      btWideChar:
        FValue.twidechar := WideChar((Val + #0)[1]);
      btWideString:
        TbtWideString(FValue.twidestring) := TbtWideString(val);
      btUnicodeString:
        TbtUnicodeString(FValue.tunistring) := TbtUnicodeString(val);
      {$ENDIF}
    else
      raise EPSCompilerException.Create(RPS_ConstantValueMismatch);
    end;
...
begin
  ...
  begin
    case FValue.FType.BaseType of
      btString:
        TbtString(FValue.tstring) := TbtString(val);
      btWideChar:
        FValue.twidechar := val;
      btWideString:
        TbtWideString(FValue.twidestring) := val;
      btUnicodeString:
        TbtUnicodeString(FValue.tUniString) := val;
      else
        raise EPSCompilerException.Create(RPS_ConstantValueMismatch);
    end;
  end
...
begin
  case Error of
    ecUnknownIdentifier:
      Result := TbtString(Format (RPS_UnknownIdentifier, [Param]));
    ecIdentifierExpected:
      Result := TbtString(RPS_IdentifierExpected);
    ecCommentError:
      Result := TbtString(RPS_CommentError);
    ecStringError:
      Result := TbtString(RPS_StringError);
    ecCharError:
      Result := TbtString(RPS_CharError);
    ecSyntaxError:
      Result := TbtString(RPS_SyntaxError);
    ecUnexpectedEndOfFile:
      Result := TbtString(RPS_EOF);
    ecSemicolonExpected:
      Result := TbtString(RPS_SemiColonExpected);
    ecBeginExpected:
      Result := TbtString(RPS_BeginExpected);
    ecPeriodExpected:
      Result := TbtString(RPS_PeriodExpected);
    ecDuplicateIdentifier:
      Result := TbtString(Format (RPS_DuplicateIdent, [Param]));
    ecColonExpected:
      Result := TbtString(RPS_ColonExpected);
    ecUnknownType:
      Result := TbtString(Format (RPS_UnknownType, [Param]));
    ecCloseRoundExpected:
      Result := TbtString(RPS_CloseRoundExpected);
    ecTypeMismatch:
      Result := TbtString(RPS_TypeMismatch);
    ecInternalError:
      Result := TbtString(Format (RPS_InternalError, [Param]));
    ecAssignmentExpected:
      Result := TbtString(RPS_AssignmentExpected);
    ecThenExpected:
      Result := TbtString(RPS_ThenExpected);
    ecDoExpected:
      Result := TbtString(RPS_DoExpected);
    ecNoResult:
      Result := TbtString(RPS_NoResult);
    ecOpenRoundExpected:
      Result := TbtString(RPS_OpenRoundExpected);
    ecCommaExpected:
      Result := TbtString(RPS_CommaExpected);
    ecToExpected:
      Result := TbtString(RPS_ToExpected);
    ecIsExpected:
      Result := TbtString(RPS_IsExpected);
    ecOfExpected:
      Result := TbtString(RPS_OfExpected);
    ecCloseBlockExpected:
      Result := TbtString(RPS_CloseBlockExpected);
    ecVariableExpected:
      Result := TbtString(RPS_VariableExpected);
    ecStringExpected:
      Result := TbtString(RPS_StringExpected);
    ecEndExpected:
      Result := TbtString(RPS_EndExpected);
    ecUnSetLabel:
      Result := TbtString(Format (RPS_UnSetLabel, [Param]));
    ecNotInLoop:
      Result := TbtString(RPS_NotInLoop);
    ecInvalidJump:
      Result := TbtString(RPS_InvalidJump);
    ecOpenBlockExpected:
      Result := TbtString(RPS_OpenBlockExpected);
    ecWriteOnlyProperty:
      Result := TbtString(RPS_WriteOnlyProperty);
    ecReadOnlyProperty:
      Result := TbtString(RPS_ReadOnlyProperty);
    ecClassTypeExpected:
    ...
    ecUnsatisfiedForward:
      Result := TbtString(Format (RPS_UnsatisfiedForward, [Param]));
    ecForwardParameterMismatch:
      Result := TbtString(RPS_ForwardParameterMismatch);
    ecInvalidnumberOfParameters:
      Result := TbtString(RPS_InvalidNumberOfParameter);
    ecInvalidEventParameters:
      Result := TbtString(RPS_InvalidEventParameters);
    ecTaskVarParameters:
      Result := TbtString(RPS_TaskVarParameters);
    ecEventsCanNotBeUsedAsVariables:
      Result := TbtString(RPS_EventsCanNotBeUsedAsVariables);
    {$IFDEF PS_USESSUPPORT}
    ecNotAllowed :
      Result := TbtString(Format(RPS_NotAllowed, [Param]));
    ecUnitNotFoundOrContainsErrors:
      Result := TbtString(Format(RPS_UnitNotFound, [Param]));
    ecCrossReference:
      Result := TbtString(Format(RPS_CrossReference, [Param]));
    {$ENDIF}
  else
...
begin
  case Hint of
    ehVariableNotUsed:
      Result := TbtString(Format (RPS_VariableNotUsed, [Param]));
    ehFunctionNotUsed:
      Result := TbtString(Format (RPS_FunctionNotUsed, [Param]));
    ehCustomHint:
      Result := Param;
  else
...
begin
  case Warning of
    ewCustomWarning:
      Result := Param;
    ewCalculationAlwaysEvaluatesTo:
      Result := TbtString(Format (RPS_CalculationAlwaysEvaluatesTo, [Param]));
    ewIsNotNeeded:
      Result := TbtString(Format (RPS_IsNotNeeded, [Param]));
    ewAbstractClass:
      Result := TbtString(RPS_AbstractClass);
  else
...
var
  V: TPSVar;
begin
  V := Sender.AddVariableN(VarName, VarType);
  if V = nil then
  begin
    Result := False;
    Exit;
  end;
  SetVarExportName(V, FastUpperCase(VarName));
  V.Use;
  Result := True;
end;
...
begin
  ...
  try
    ...
    begin
      Result := False;
      {$IFDEF DEBUG}
      raise EPSCompilerException.CreateFmt(RPS_UnableToRegister, [Decl]);
      {$ENDIF}
      Exit;
    end;

    if FT in [mtfEvent, mtfTask] then
    begin
      Result := False;
      {$IFDEF DEBUG}
      raise EPSCompilerException.CreateFmt(RPS_UnableToRegister, [Decl]);
    ...
    if ft = mftConstructor then
      p := TPSDelphiClassItemConstructor.Create(Self, Decl)
    else
      p := TPSDelphiClassItemMethod.Create(Self, Decl);
    p.OrgName := DOrgName;
    p.Decl.Assign(DDecl);
...
begin
  pt := PropertyType;
  p := TPSDelphiClassItemProperty.Create(Self, PropertyName + ' ' + PropertyType);
  p.AccessType := PropAC;
...
var
  ...
  a: TPSPropType;
begin
  if (Fclass = nil) or (Fclass.ClassInfo = nil) then
    Exit;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        raise EPSCompilerException.CreateFmt(RPS_NotProperty, [Name]);
    end;
  end;
...
begin
  case SetType.BaseType of
    btEnum:
      Result := TPSEnumType(setType).HighValue + 1;
    btChar,
    btU8:
      Result := 256;
  else
...
begin
  ...
  if Func in [mtfEvent, mtfTask] then
  begin
    FItems.Add(m); // in any case, add a dummy item
    Result := False;
    Exit;
