...
begin
  ...
  if (Result > 0) and (FAllNodes[0] <> FChilds[0]) then
    Dec(Result);
end;
...
begin
  if (Int1.Hi <> Int2.Hi) then
    ...
    Result := UInt64LT(Int1.Lo, Int2.Lo);
end;
//------------------------------------------------------------------------------
...
begin
  ...
  if UInt64LT(Result.Lo, Int1.Lo) then
    Inc(Result.Hi);
end;
//------------------------------------------------------------------------------
...
begin
  Result.Hi := Int1.Hi - Int2.Hi;
  Result.Lo := Int1.Lo - Int2.Lo;
  if UInt64GT(Result.Lo, Int1.Lo) then
    Dec(Result.Hi);
end;
//------------------------------------------------------------------------------
...
begin
  ...
  if Dividend.Hi < 0 then
    Int128Negate(Dividend);
  if Divisor.Hi < 0 then
    Int128Negate(Divisor);
  ...
  for I := I to Len -1 do
    ...
    begin
      if polys[I][J].X < Result.Left then
        Result.Left := polys[I][J].X
      else if polys[I][J].X > Result.Right then
        Result.Right := polys[I][J].X;
      if polys[I][J].Y < Result.Top then
        Result.Top := polys[I][J].Y
      else if polys[I][J].Y > Result.Bottom then
        Result.Bottom := polys[I][J].Y;
    end;
end;
//------------------------------------------------------------------------------
...
begin
  if currentY = Edge.Top.Y then
    Result := Edge.Top.X
  else if Edge.Top.X = Edge.Bot.X then
    Result := Edge.Bot.X
  else
    Result := Edge.Bot.X + Round(Edge.Dx * (currentY - Edge.Bot.Y));
end;
//------------------------------------------------------------------------------
...
begin
  if (Pt.Z <> 0) or not assigned(ZFillFunc) then
    Exit
  else if PointsEqual(Pt, E1.Bot) then
    Pt.Z := E1.Bot.Z
  else if PointsEqual(Pt, E2.Bot) then
    Pt.Z := E2.Bot.Z
  else if PointsEqual(Pt, E1.Top) then
    Pt.Z := E1.Top.Z
  else if PointsEqual(Pt, E2.Top) then
    Pt.Z := E2.Top.Z
  else
...
begin
  while True do
  begin
    while not PointsEqual(E.Bot, E.Prev.Bot) or PointsEqual(E.Curr, E.Top) do
      E := E.Next;
    if (E.Dx <> Horizontal) and (E.Prev.Dx <> Horizontal) then
      Break;
    while (E.Prev.Dx = Horizontal) do
      E := E.Prev;
    E2 := E; //E2 == first horizontal
    while (E.Dx = Horizontal) do
      E := E.Next;
...
begin
  ...
  begin
    ...
    begin
      ...
      while (E <> Result) and (E.Dx = Horizontal) do
        E := E.Prev;
    end
    else
    begin
  ...
  begin
    ...
    begin
      ...
      Horz := Result;
      while (Horz.Prev.Dx = Horizontal) do
        Horz := Horz.Prev;
  ...
  begin
    ...
    begin
      ...
      if (Horz.Next.Top.X = Result.Prev.Top.X) or
        (Horz.Next.Top.X > Result.Prev.Top.X) then
          Result := Horz.Next;
    end;
    while E <> Result do
    begin
...
{$ELSE}
  if not Closed then
    raise exception.Create(rsOpenPath2);
{$ENDIF}
  ...
  HighI := High(Path);
  if Closed then
    while (HighI > 0) and PointsEqual(Path[HighI],Path[0]) do
      Dec(HighI);
  while (HighI > 0) and PointsEqual(Path[HighI],Path[HighI - 1]) do
    Dec(HighI);
  if (Closed and (HighI < 2)) or (not Closed and (HighI < 1)) then
    Exit;
...
begin
  ...
  for I := 0 to high(Paths) do
    if AddPath(Paths[I], PolyType, Closed) then
      Result := True;
end;
//------------------------------------------------------------------------------
...
begin
  ...
  begin
    sb := FScanbeam;
    while Assigned(sb.Next) and (Y <= sb.Next.Y) do
      sb := sb.Next;
    if Y <> sb.Y then
    begin
      newSb.Next := sb.Next;
...
begin
  Result := False;
  if FCurrentLocMinIdx = FLocMinList.Count then
    Exit;
...
begin
  ...
  if HasOpenPaths then
    raise Exception.Create(rsOpenPath3);
  try
    try
      FExecuteLocked := True;
...
begin
  Result := False;
  if FExecuteLocked or not Assigned(PolyTree) then
    Exit;
  try
    try
      FExecuteLocked := True;
...
begin
  ...
  if not Assigned(OutRec.FirstLeft) or
    ((OutRec.IsHole <> OutRec.FirstLeft.IsHole) and
      Assigned(OutRec.FirstLeft.Pts)) then
        Exit;
...
begin
  ...
  case Pft of
    pftEvenOdd:
      Result := (Edge.WindDelta <> 0) or (Edge.WindCnt = 1);
    pftNonZero:
...
begin
  ...
  begin
    ...
    if not assigned(Lb) or not assigned(Rb) then
      Continue;
...
begin
  ...
  P := btmPt2.Next;
  while PointsEqual(P.Pt, btmPt2.Pt) and (P <> btmPt2)
    do P := P.Next;
...
begin
  ...
  if OutRec1RightOfOutRec2(OutRec1, OutRec2) then
    HoleStateRec := OutRec2
  else if OutRec1RightOfOutRec2(OutRec2, OutRec1) then
  ...
  try
    ...
    begin
      ...
      if (OutRec.IsOpen and (cnt < 2)) or
        (not outRec.IsOpen and (cnt < 3)) then
        Continue;
...
begin
  ...
  if IsHorizontal and
     PointsEqual(Jr.OffPt, Jr.OutPt1.Pt) and
  ...
  begin
    ...
    Op1b := Jr.OutPt1.Next;
    while (Op1b <> Op1) and PointsEqual(Op1b.Pt, Jr.OffPt) do
      Op1b := Op1b.Next;
    Reverse1 := (Op1b.Pt.Y > Jr.OffPt.Y);
    Op2b := Jr.OutPt2.Next;
    while (Op2b <> Op2) and PointsEqual(Op2b.Pt, Jr.OffPt) do
      Op2b := Op2b.Next;
  ...
  begin
    ...
    else if (Op2.Pt.X >= Left) and (Op2.Pt.X <= Right) then
    begin
      Pt := Op2.Pt; DiscardLeftSide := Op2.Pt.X > Op2b.Pt.X;
    end
    else if (Op1b.Pt.X >= Left) and (Op1b.Pt.X <= Right) then
    begin
  ...
  begin
    ...
    Op1b := Op1.Next;
    while PointsEqual(Op1b.Pt, Op1.Pt) and (Op1b <> Op1) do
      Op1b := Op1b.Next;
    ...
    begin
      ...
      if (Op1b.Pt.Y > Op1.Pt.Y) or
        not SlopesEqual(Op1.Pt, Op1b.Pt, Jr.OffPt, FUse64BitRange) then
        Exit;
    end;
    Op2b := Op2.Next;
    while PointsEqual(Op2b.Pt, Op2.Pt) and (Op2b <> Op2) do
      Op2b := Op2b.Next;
    ...
    begin
      ...
      if (Op2b.Pt.Y > Op2.Pt.Y) or
        not SlopesEqual(Op2.Pt, Op2b.Pt, Jr.OffPt, FUse64BitRange) then
        Exit;
    end;

    if (Op1b = Op1) or (Op2b = Op2) or (Op1b = Op2b) or
      ((OutRec1 = OutRec2) and (Reverse1 = Reverse2)) then
      Exit;
...
begin
  ...
  begin
    ...
    else if (orec.FirstLeft = InnerOutRec) or (orec.FirstLeft = OuterOutRec) then
      orec.FirstLeft := orfl;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  ...
  begin
    ...
    if not Assigned(OutRec1.Pts) or not Assigned(OutRec2.Pts) then
      Continue;
    ...
    if OutRec1 = OutRec2 then
      HoleStateRec := OutRec1
    else if OutRec1RightOfOutRec2(OutRec1, OutRec2) then
      HoleStateRec := OutRec2
    else if OutRec1RightOfOutRec2(OutRec2, OutRec1) then
      HoleStateRec := OutRec1
    else
    ...
    if not JoinPoints(Jr, OutRec1, OutRec2) then
      Continue;
    ...
    begin
      ...
      else if Poly2ContainsPoly1(OutRec1.Pts, OutRec2.Pts) then
      begin
...
begin
  ...
  begin
    ...
    inc(I);
    Op := OutRec1.Pts;
    if not assigned(Op) or OutRec1.IsOpen then
      Continue;
    repeat //for each Pt in Path until duplicate found do ...
      Op2 := Op.Next;
      while (Op2 <> OutRec1.Pts) do
      begin
        if (PointsEqual(Op.Pt, Op2.Pt) and (Op2.Next <> Op) and (Op2.Prev <> Op)) then
        begin
...
procedure TClipperOffset.AddPath(const Path: TPath; JoinType: TJoinType;
  EndType: TEndType);
var
  I, J, K, HighI: Integer;
  NewNode: TPolyNode;
...
begin
  ...
  if EndType in [etClosedLine, etClosedPolygon] then
    while (HighI > 0) and PointsEqual(Path[0], Path[HighI]) do
      dec(HighI);
  ...
  if EndType <> etClosedPolygon then
    Exit;
...
begin
  ...
  if FMiterLimit > 2 then
    FMiterLim := 2 / Sqr(FMiterLimit)
  else
    FMiterLim := 0.5;
...
begin
  ...
  begin
    ...
    else if PointsAreClose(op.Prev.Pt, op.Next.Pt, DistSqrd) then
    begin
      ExcludeOp(op.Next);
      op := ExcludeOp(op);
      Dec(Len, 2);
